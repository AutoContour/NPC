...
class dicomSubject(object):
    def __init__(self, subject_folder_path):
        ...
        modalities = os.listdir(subject_folder_path)
        modalities.sort()
        self.modalities = os.listdir(subject_folder_path)
        self.modalities.sort()
...
def load_train_negative(batch_size, n_epochs, patchSize):
    ...
    for epoch in xrange(n_epochs*case_num):
        ...
        train_x = np.zeros([batch_size, 4, 2*patchX, 2*patchY, 2*patchZ])
